...
void AudioRecorder::ProcessSLCallback(SLAndroidSimpleBufferQueueItf bq) {
    ...
    if (!pause) {
        fwrite(dataBuf->buf_, dataBuf->size_, 1, recordFile);
    }


//    __android_log_print(ANDROID_LOG_ERROR, "audio-echo", "write");
    sample_buf *freeBuf;
...
AudioRecorder::AudioRecorder(SampleFormat *sampleFormat, SLEngineItf slEngine, const char *path) :
        freeQueue_(nullptr), recQueue_(nullptr), devShadowQueue_(nullptr),
        callback_(nullptr) {


    recordFile = fopen(path, "w+");
    if (recordFile == NULL) {
        __android_log_print(ANDROID_LOG_DEBUG, "audio_echo", "file not open");
    }
...
//    result = (*recObjectItf_)->GetInterface(recObjectItf_, SL_IID_VOLUME, &slVolumeItf);
...
uint16_t AudioRecorder::getVolume() {
    SLmillibel pLevel = 0;
    if (slVolumeItf != NULL) {
        SLresult sLresult = (*slVolumeItf)->GetVolumeLevel(slVolumeItf, &pLevel);
