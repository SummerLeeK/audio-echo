...
AudioPlayer::AudioPlayer(SampleFormat *sampleFormat, SLEngineItf slEngine) :
    ...
    result = (*slEngine)->CreateAudioPlayer(slEngine, &playerObjectItf_, &audioSrc, &audioSnk,
                                            2, ids, req);
//    SLASSERT(result);
    __android_log_print(ANDROID_LOG_ERROR, "DEBUG", "ss %d", result);
    // realize the player
    ...
    result = (*playerObjectItf_)->GetInterface(playerObjectItf_, SL_IID_VOLUME, &slVolumeItf);
...
SLresult AudioPlayer::setPlayerMute() {
//    SLuint8 pNumChannels;
    SLresult sLresult = (*slVolumeItf)->SetMute(slVolumeItf, SL_BOOLEAN_TRUE);

    __android_log_print(ANDROID_LOG_ERROR, "DEGUG", "mute : %d", sLresult);
    return sLresult;
}

SLresult AudioPlayer::setPlayerNotMute() {
//    SLuint8 pNumChannels;
    SLresult sLresult = (*slVolumeItf)->SetMute(slVolumeItf, SL_BOOLEAN_FALSE);
    __android_log_print(ANDROID_LOG_ERROR, "DEGUG", "mute : %d", sLresult);
    return sLresult;
}

SLmillibel AudioPlayer::getVolumeLevell() {
    SLmillibel volume;
    SLresult sLresult = (*slVolumeItf)->GetVolumeLevel(slVolumeItf, &volume);
    __android_log_print(ANDROID_LOG_ERROR, "DEBUG", "volume : %d", volume);
    assert(SL_RESULT_SUCCESS == sLresult);
    return volume;
}

SLmillibel AudioPlayer::setVolumeLevel(SLmillibel volume) {

    SLresult sLresult = (*slVolumeItf)->SetVolumeLevel(slVolumeItf, volume);
    __android_log_print(ANDROID_LOG_ERROR, "DEBUG", "volume : %d", volume);
    assert(SL_RESULT_SUCCESS == sLresult);
    return volume;
}

SLmillibel AudioPlayer::getMaxVolumeLevel() {
    SLmillibel volume;
    SLresult sLresult = (*slVolumeItf)->GetMaxVolumeLevel(slVolumeItf, &volume);
    __android_log_print(ANDROID_LOG_ERROR, "DEBUG", "volume : %d", volume);
    assert(SL_RESULT_SUCCESS == sLresult);
    return volume;
}

SLresult AudioPlayer::getMaxVolumeLevel(SLmillibel volume) {
    SLresult result = (*slVolumeItf)->SetVolumeLevel(slVolumeItf, volume);
